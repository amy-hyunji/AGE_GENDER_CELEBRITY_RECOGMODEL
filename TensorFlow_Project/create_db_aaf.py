...
def get_args():
    parser = argparse.ArgumentParser(description="This script creates database for training from the All-Age-Faces dataset.",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("--input", "-i", type=str, required=True,
                        help="path to the All-Age-Faces image directory")
...
def main():
    ...
    for i, image_path in enumerate(tqdm(image_dir.glob("*.jpg"))):
        image_name = image_path.name  # [person_id]A[age].jpg
        person_id, rest = image_name.split("A")[:2]
        age = rest[0:1]
        gender = int(person_id) < 7380
